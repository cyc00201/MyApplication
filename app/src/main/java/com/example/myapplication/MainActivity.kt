...
class MainActivity : AppCompatActivity() {
    ...
    private fun openFile(data: Intent) {
        ...
        val bitmap: Bitmap = Bitmap.createScaledBitmap(
            ...
            paintboard.getbasebitmao().width,
            paintboard.getbasebitmao().height,
            true
        )

        paintboard.setbase(bitmap)
        if(layer_spinner.selectedItemId > 0){
    ...
    private fun saveClickHandler() {
        ...
        if (checkWritable()) {
            try {
                ...
                Bitmap.createBitmap(paintboard.getBitmap()).compress(Bitmap.CompressFormat.JPEG, 100, stream)
